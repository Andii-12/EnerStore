    ...
    <div style={{ margin: '48px 0' }}>
      <div style={{ background: '#fff', border: '1px solid #e5e7eb', borderRadius: 18, boxShadow: '0 2px 12px rgba(8,15,70,0.06)', padding: 32, marginBottom: 48 }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24 }}>
          ...
          <button style={{ background: 'var(--color-accent)', color: '#fff', border: 'none', padding: '12px 32px', borderRadius: 8, fontSize: 16, fontWeight: 600, cursor: 'pointer', transition: 'background 0.2s' }}>
            Бүгдийг үзэх
          </button>
        </div>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 24 }}>
          {newProducts.map(product => (
            <div key={product._id} className="new-product-card" style={{ background: '#fff', border: '1px solid #eee', borderRadius: 12, boxShadow: '0 2px 8px rgba(0,0,0,0.04)', display: 'flex', flexDirection: 'column', padding: 18, minHeight: 280, position: 'relative', cursor: 'pointer', transition: 'box-shadow 0.18s, border 0.18s, transform 0.18s' }}>
              {/* Product image */}
...
function SaleProductsSection({ products }) {
  const saleProducts = products.filter(p => p.originalPrice && p.price < p.originalPrice);
